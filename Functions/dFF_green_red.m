function [dFoverFWhole_x465A, dFoverFWhole_x560B] = dFF_green_red(data, varargin)
...
%normalizes 560 channel to time
datathree=data.streams.x560B.data(round(Start):round(Stop));
time=1:length(data.streams.x560B.data(round(Start):round(Stop)));
reg = polyfit(time, datathree, 1);
f0 = reg(1)*time + reg(2);
signal_df_f0 = (datathree - f0)./f0 * 100;

%normalizes 405 channel to time
datafour=data.streams.x405B.data(round(Start):round(Stop));
time=1:length(data.streams.x405B.data(round(Start):round(Stop)));
reg = polyfit(time, datafour, 1);
f0 = reg(1)*time + reg(2);
reference_df_f0 = (datafour - f0)./f0 * 100;

%subtracts the two
dFoverFWhole_x560B = signal_df_f0 - reference_df_f0;
...
dFoverFWhole_x560B=filter(c,d,dFoverFWhole_x560B);